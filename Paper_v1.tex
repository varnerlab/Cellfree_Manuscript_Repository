...
The glucose uptake rate was bounded in the range [0,40 mM/h] according to experimental data;
...
G_{p}^{*}+\sum_{i}\eta_{i}iTP &\longrightarrow& mRNA+G_{p}+R_{1}+2\sum_{i}\eta_{i}P \qquad{i=A,C,G,U}\\
mRNA &\longrightarrow& \sum_{i}\eta_{i}iMP \\
mRNA+R_{2} &\longrightarrow& R_{2}^{*} \\
\alpha_{j}X_{j}+\alpha_{j}tRNA+\alpha_{j}ATP &\longrightarrow& \alpha_{j}XtRNA_{j}+\alpha_{j}AMP+2\alpha_{j}P \qquad{j=1,2,\hdots,20}\\
R_{2}^{*}+\sum_{j}\alpha_{j}\Big(XtRNA_{j}+2GTP\Big) &\longrightarrow& Protein+R_{2}+mRNA\\
&& \quad +\sum_{j}\alpha_{j}\Big(tRNA+2GDP+2P\Big)
\end{eqnarray*}
where $G_{p}$ denotes the gene of protein $p$, $R_{1}$ denotes the concentration of RNA polymerase, $G_{p}^{*}$ denotes the gene bounded by the RNA polymerase, $\eta_{i}$ and $ \alpha_{j}$ denotes the coefficient for species $i$ and $j$ respectively, $P$ denotes phosphate, $R_{2}$ denotes the ribosome concentration, $R_{2}^{*}$ denotes bounded ribosome, and $X_{j}$ denotes the amino acid species $j$. 
