...
Our model predicts a carbon yield of 0.0871 for chloramphenicol acetyltransferase synthesis, equal to 45\% of the theoretical maximum calculated with constraint-based modeling.
It also predicts most carbon flux going through glycolysis and the TCA cycle, while the constraint-based approach predicts most carbon flux going through the Entner-Doudoroff pathway and only part of glycolysis.
Our sensitivity analysis indicates that chloramphenicol acetyltransferase synthesis is most sensitive to the parameters associated with the synthesis of GTP, GMP, and amino acids, and relatively insensitive to glucose and oxygen levels and allosteric control.
However, allosteric control is important for the model to accurately predict the dynamics of some organic acids.
This modeling framework represents a significant achievement toward genome-scale, dynamic modeling of \textit{E. coli} cell-free protein synthesis.
...
We constructed an initial model of cell-free \textit{E. coli} metabolism by removing the growth-associated processes from the genome-scale model of Palsson and coworkers \cite{2000_edwards_palsson_PNAS} and adding reactions for the synthesis of chloramphenicol acetyltransferase (CAT), a model protein for which we have a comprehensive training dataset \cite{2005_calhoun_BiotechnologyProgress}. 
This initial model captured the core metabolism (glycolysis, pentose phosphate, Enter-Doudoroff, TCA cycle), as well as the synthesis of energy species, amino acids, and CAT  (Fig.~\ref{fig:Network}).
...
The model was trained against the training dataset, which included measurements of glucose, CAT, organic acids (pyruvate, lactate, acetate, succinate, malate), energy species (AXP, GXP, CXP, UXP), and 18 of the 20 proteinogenic amino acids.
We generated an ensemble of 18,000 parameter sets by minimizing the error between the training dataset and the metabolite concentrations predicted by the model.
Out of these 18,000 sets, we defined the set with the lowest error value as our best-fit set.
The ensemble predicts most carbon flux going through glycolysis and the TCA cycle, and to a lesser extent the Entner-Doudoroff pathway  (Fig.~\ref{fig:Network}, bottom two rows of gray boxes).
Pentose phosphate is much less utilized.
