...
G_{p}^{*}+\sum_{k}\eta_{k}\cdot kTP &\longrightarrow& mRNA+G_{p}+R_{1}+2\sum_{k}\eta_{k}\cdot Pi \qquad{k=A,C,G,U}\\
mRNA &\longrightarrow& \sum_{k}\eta_{k}\cdot kMP \\
mRNA+R_{2} &\longrightarrow& R_{2}^{*} \\
\alpha_{j}\cdot AA_{j}+\alpha_{j}\cdot tRNA+\alpha_{j}\cdot ATP &\longrightarrow& \alpha_{j}\cdot AA_{j}:tRNA_{j}+\alpha_{j}\cdot AMP+2\alpha_{j}\cdot Pi \qquad{j=1,2,\hdots,20}\\
