...
CAT production was most dependent upon the abundance of cofactors such as ATP, GTP, NADH and NADPH and the metabolite pyruvate.
...
Taking into account these three reactions, and the next 16 most important reactions (section B), we saw a common theme of reactions involving the cofactors ATP, GTP, NADH, and NADPH
as well as the metabolites pyruvate and glutamate.
Glutamate was important as a precursor for the synthesis of other amino acids required by CAT production.
...
The combination of certain reactions had a greater effect on CAT production than that of the individual reactions by themselves.
...
\textcolor{red}{NICK: ARE THERE EXAMPLES OF POSITIVE SYNERGY?}

The overall system state was also sensitive to cofactors and substrates; however, instead of pyruvate and glutamate, the substrates driving metabolism were glucose and pyruvate.
...
For example, the first-order effect of alanine synthesis was large; it consumed both pyruvate and glutamate, two key species in the network.
However, there were enough reactions that, when paired with alanine synthesis, had little effect on the model;
for example, malic enzyme produces pyruvate that can be consumed by alanine synthesis.
...
Knocking out the \textit{gnd} reaction decreased acetate flux by more than half.
In addition, less uptake of amino acids were required which increased the carbon yield of CAT by 2\% (up to approximately 8\%) compared to the control (no knockouts).
The simulation showed an increase in oxidative phosphorylation flux and the majority of flux traveling through the pentose phosphate pathway.
A second simulation with both \textit{gnd} and phosphate acetyltransferase knocked out,
showed less carbon was needed to meet the translation bound, which increased the carbon yield by 6\% compared to the control (up to approximately 14\%).
In the dual knockout, flux towards acetate was mostly coming from amino acid degradation.
Sixty percent of the carbon flux traveled through glycolysis, while the remaining traveled through the pentose phosphate pathway.
...
\textcolor{red}{MIKE: CAN WE GET A FIGURE THAT SHOWS THE KO FLUXES -and- WHAT ARE THE ERRORS ON THE YIELDS?}
...
The objective of the calculation is the maximization of CAT translation rate, $w_{TL}$.
...
The transcription and translation rates were modeled using the template reactions:
\begin{eqnarray*}
G_{p}+R_{1} &\longrightarrow& G_{p}^{*} \\
G_{p}^{*}+\sum_{k}\eta_{k}\cdot kTP &\longrightarrow& mRNA+G_{p}+R_{1}+\\
&& \qquad 2\sum_{k}\eta_{k}\cdot Pi \qquad{k=A,C,G,U}\\
mRNA &\longrightarrow& \sum_{k}\eta_{k}\cdot kMP \\
mRNA+R_{2} &\longrightarrow& R_{2}^{*} \\
...
R_{2}^{*}+\sum_{j}\alpha_{j}\cdot \Big(AA_{j}:tRNA_{j}+2\cdot GTP\Big) &\longrightarrow& \mathcal{P}+R_{2}+mRNA+\\
&& \quad +\sum_{j}\alpha_{j}\Big(tRNA+2\cdot GDP+2\cdot Pi\Big)
\end{eqnarray*}
where $G_{p}$ denotes the gene of protein $p$,
$R_{1}$ denotes the concentration of RNA polymerase,
$G_{p}^{*}$ denotes the gene bounded by the RNA polymerase,
$\eta_{i}$ and $ \alpha_{j}$ denote the coefficients for species $i$ and $j$ respectively,
$P_{i}$ denotes inorganic phosphate, $R_{2}$ denotes the ribosome concentration, $R_{2}^{*}$ denotes bounded ribosome, and $X_{j}$ denotes the amino acid species $j$.

The transcription rate ($w_{TX}$) was fixed in the ssFBA calculation as:
\begin{equation}
	w_{TX} = \left[R_{1}\left(\frac{v_{TX}}{l_{G}}\right)\left(\frac{K_{T7}}{1 + K_{T7}}\right)\right]\left(\frac{G}{K_{TX}+G}\right)
\end{equation}
The first term in the brackets denotes the maximum rate of transcription, $v_{TX}$ denotes the RNA polymerase elongation rate (nt/hr), $l_{G}$ denotes the gene length in nucleotides, and the last term describes T7 promoter activity, where $K_{T7}$ quantifies T7 RNA polymerase binding \cite{Moon:2012aa}.
The second term denotes the gene saturation term, where $G$ denotes the gene concentration and $K_{TX}$ denotes a saturation coefficient.
The CAT gene and protein sequences were determined from literature.
The translation rate ($w_{TL}$) was bounded by:
 \begin{equation}
	0\leq w_{TL} \leq \left[K_{P} R_{2}\left(\frac{v_{TL}}{l_{P}}\right)\right]\left(\frac{mRNA_{SS}}{K_{TL}+mRNA_{SS}}\right)
\end{equation}
The first term in brackets denotes the maximum translation rate, where $K_{P}$ denotes the polysome amplification constant, $v_{TL}$ denotes the ribosome elongation rate (amino acids per hour), $l_{P}$ denotes the number of amino acids in the protein of interest, and $mRNA_{SS}$ denotes the steady-state mRNA concentration:
\begin{equation}
...
\textcolor{red}{JV: UPDATE THIS TEXT}
