...
The ensemble captured all of the training data, especially the central carbon metabolism.
CAT was produced with a carbon yield of 7\% and an energy efficiency of 5\%, suggesting that CAT production could be further optimized.
...
Model parameters were estimated from measurements of glucose, organic acids, energy species, amino acids, and the protein product, chloramphenicol acetyltransferase (CAT).
Characteristic values for model parameters and metabolite initial conditions, estimated from literature, were used to constrain the parameter estimation problem.
...
25\% of the carbon was accumulated as organic acids (lactate, acetate, succinate, and malate), leaving 61\% as accumulation of other byproducts, species for which data do not exist.
...
These results show that there is much room for improvement of the efficiency of CFPS.
This demonstrates that CAT carbon yield could be improved by diverting flux away from some of the less efficient or altogether unnecessary metabolic pathways.
%pathways of energy generation
...
% and production of CAT.
% central carbon metabolism is exhuasted within 1.5 hours, leading to low CAT production.
...
% with the first phase operating on glucose and the second phase operating on pyruvate.
...
% sustained by pyruvate and glutamate
While CAT synthesis can be powered by other substrates, the rate of production about half (\texttildelow 5 $\mu$M/h, as opposed to \texttildelow 10 $\mu$M/h).
...
%CAT synthesis is the most important, followed by oxidative phosphorylation and the glutamate and pyruvate consuming reactions, as well as cofactor reactions which are necessary to drive CAT synthesis.
...
%catalytic rate bounds of 0 and 27,780 s$^{-1}$.
%\cite{Milo:2009aa}
Model parameters were constrained by literature.
Rate maxima were bounded within one order of magnitude of the literature value where available (Table~\ref{tbl:Rate_maxima}).
All other rate maxima were bounded within two orders of magnitude of the geometric mean of the available values.
...
%\phantom{i}\textbf{Transcription/Translation} & \textbf{AA-tRNA complex (mM)} & \textbf{Kcat (h$^-1$)} & \textbf{Vmax (mM/h)} & \textbf{Reference} \\ \hline
...
\begin{table}
\centering
 \caption{Reference values for reaction rate maxima (V$_{max}$) from literature. V$_{max}$ values calculated from original turnover numbers (K$_{cat}$) and characteristic enzyme concentration, extrapolated to cell-free using a dilution factor of 30. Characteristic rate maximum for all other reactions calculated as geometric mean of calculated rate maxima.}
 \renewcommand{\arraystretch}{1.3}
 \begin{tabular}{llrrr} \toprule
\smallskip
 & & \parbox{2.1 cm}{\textbf{Original value ($\mu$M)}} & \parbox{3.1 cm}{\textbf{Cell-free \phantom{abc} equivalent (nM)}} & \textbf{Reference} \\ \hline
\smallskip
\rule{0pt}{3ex} \parbox{4.2 cm}{Characteristic enzyme concentration} & & 5 & 167 & BNID 100735 \\ \toprule
...
%\phantom{i}\textbf{Transcription/Translation} & \textbf{AA-tRNA (mM)} & \textbf{Kcat (h$^-1$)} & \textbf{Vmax (mM/h)} & \textbf{Reference} \\ \hline
\phantom{i}tRNA charging & 0.03 & 14040 & 4212 & BNID 104980 \\ \hline
% \bottomrule
 \end{tabular}
\label{tbl:Rate_maxima}
\end{table}
...
\phantom{i}Cellular volume & 3.881e-16 & XXX \\ \hline
 \toprule
 & \textbf{Concentration (mM)} & \textbf{Reference} \\ \hline
\phantom{i}CAT gene & 5.19e-6 & XXX \\ \hline
\phantom{i}RNA polymerase & 6.75e-5 & \cite{Garamella:2016aa} \\ \hline
\phantom{i}Ribosome & 2e-3 & \cite{Garamella:2016aa} \\ \hline
\phantom{i}tRNA & 1.6 & BNID 108611 \\ \hline
\phantom{i}NAD & 1.47 & XXX \\ \hline
\phantom{i}NADH & 0.1 & XXX \\ \hline
\phantom{i}NADP & 0.195 & XXX \\ \hline
\phantom{i}NADPH & 0.062 & XXX \\ \hline
\phantom{i}G6P & 3.48 & XXX \\ \hline
\phantom{i}F6P & 0.6 & XXX \\ \hline
\phantom{i}FDP & 0.272 & XXX \\ \hline
\phantom{i}DHAP & 0.167 & XXX \\ \hline
\phantom{i}G3P & 0.218 & XXX \\ \hline
\phantom{i}13DPG & 0.008 & XXX \\ \hline
\phantom{i}3PG & 2.13 & XXX \\ \hline
\phantom{i}2PG & 0.399 & XXX \\ \hline
\phantom{i}6PGL & 0.808 & XXX \\ \hline
\phantom{i}6PGC & 0.808 & XXX \\ \hline
\phantom{i}RU5P & 0.111 & XXX \\ \hline
\phantom{i}XU5P & 0.138 & XXX \\ \hline
\phantom{i}R5P & 0.398 & XXX \\ \hline
\phantom{i}S7P & 0.276 & XXX \\ \hline
\phantom{i}E4P & 0.098 & XXX \\ \hline
\phantom{i}NH3 & 0.052425 & XXX \\ \hline
\phantom{i}ARG & 1.9326 & XXX \\ \hline
\phantom{i}H2O2 & 0.052425 & XXX \\ \hline
\phantom{i}HCO3 & 0.052425 & XXX \\ \hline
% \bottomrule
 \end{tabular}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.55\textwidth]{./Figures/Fig_6_CarbonEnergyBalances.pdf}
