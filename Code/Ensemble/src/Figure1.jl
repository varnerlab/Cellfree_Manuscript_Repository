# MassBalances.jl is set up to save the flux vector at all timepoints satisfying t < 1e-11 or 1.48 < t < 1.52 or t > 2.98
# At each timepoint, a vector of length 327 is saved to disk: the first element is the timepoint, followed by a flux value for each of the 326 reactions
# This is done for each set in the ensemble (there are 18000 such sets)

# For each set in the ensemble, we read all flux vectors and merge them into an array
# We then find the timepoint closest to 1.5 hr and save the corresponding flux vector, under the directory for that set, under the name "flux_1_5"
# We then find the timepoint closest to 3 hr and save the corresponding flux vector, under the directory for that set, under the name "flux_3"
num_dir = convert(Int64,readdlm("Ensemble/dir")[1])
for i in 1:num_dir
    # Read number of timepoints
    num_tp = convert(Int64,readdlm("./Ensemble/$i/Flux/num_tp")[1])
    # Read first flux
    rate_vector = readdlm("./Ensemble/$i/Flux/1")
    # Read all other fluxes; append to first flux
    j_range = 2:num_tp
    for j in j_range
        r = readdlm("./Ensemble/$i/Flux/$j")
        rate_vector = [rate_vector r]
    end
    # Save flux at t = 0
    tp = rate_vector[1,:]
    index = find(tp.==minimum(tp))[1]
    writedlm("./Ensemble/$i/flux_0",rate_vector[2:end,index])
    # Save flux at t = 1.5
    tp_1_5 = abs(tp-1.5)
    index = find(tp_1_5.==minimum(tp_1_5))[1]
    writedlm("./Ensemble/$i/flux_1_5",rate_vector[2:end,index])
    # Save flux at t = 3
    tp_3 = abs(tp-3.0)
    index = find(tp_3.==minimum(tp_3))[1]
    writedlm("./Ensemble/$i/flux_3",rate_vector[2:end,index])
end

# Next we load the 1.5 hr flux vector for each set and merge these into an array, and do the same for the 3 hr flux vector

Flux_early = vec(readdlm("Ensemble/1/flux_0"))
Flux_mid = vec(readdlm("Ensemble/1/flux_1_5"))
Flux_late = vec(readdlm("Ensemble/1/flux_3"))

for i in 2:num_dir
    println(i)
    flux_early = vec(readdlm("Ensemble/$i/flux_0"))
    flux_mid = vec(readdlm("Ensemble/$i/flux_1_5"))
    flux_late = vec(readdlm("Ensemble/$i/flux_3"))
...
# Then we pull the fluxes of interest from the arrays, those that corresponding to major pathways that we want to illustrate on the network figure
# In many cases we are computing one flux minus another; these are reversible reactions where both directions are catalyzed by the same enzyme
# In other cases there is only a single flux to consider
# We compute the mean and standard deviation of the fluxes across the ensemble, and save in directories called "Mid" (1.5 hr) and "Late" (3 hr)
...
# Finally, we normalize to the CAT synthesis flux, compute standard error by dividing standard deviation by sqrt(18000), and print to output
