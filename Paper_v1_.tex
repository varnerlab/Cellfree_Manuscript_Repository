...
\vspace{0.05in}
{\par \centering \large{Nicholas Horvath, Michael Vilkhovoy, Che-Hsiao Shih$^{1}$, Joseph Wayman, Kara Calhoun$^{2}$, James Swartz$^{2}$ and Jeffrey D. Varner$^{*}$}}
\vspace{0.10in}
...
{\par \centering {Purdue University, Lafayette, IN 47901}}
\vspace{0.1in}
{\par \centering {$^{2}$School of Chemical Engineering}}
...
The ensemble also captured the energy species dynamics, particularly the overall energy total (Fig.~\ref{fig:BothCarbon}, top) and the totals by base (Fig.~\ref{fig:Energy}).
...
The ensemble of 100 sets captured the CAT concentration profile which was randomly generated by sampling RNA polymerase levels, ribosome levels and elongation rates in a physiological range.
We then used sequence-specific FBA to calculate a theoretical maximum CAT yield under four different cases: unconstrained, limited oxidative phosphorylation, bounded by transcription/translation rates, and bounded by experimental data (Fig. ~\ref{fig:CATProdYield}B).
The theoretical maximum carbon yield of CAT was 0.349 for an unconstrained case and 0.194 for the transcription and translation constrained case.
Thus, we showed that our experimental dataset and best-fit parameter set were each producing CAT at 25\% of the theoretical maximum and 45\% of a theoretical physiological case.
...
In comparing the flux distributions between the unconstrained and constrained cases (Fig. ~\ref{fig:Network}), both constrained cases heavily utilize the Entner–Doudoroff pathway which may be a first viable knockout to increase CAT yield.
...
\subsection*{Sensitivity analysis on FBA system}
We conducted global sensitivity on certain fluxes' upper bounds within a constraint-based FBA system, with protein export rate as the objective function.
...
\end{equation}
where $\mathcal{D}$ denotes the number of datasets, equal to 37, $w_i$ denotes the weight of the $i$th dataset, 
%equal to 5 for the glucose, CAT, pyruvate, lactate, acetate, succinate, and malate datasets, and 1 elsewhere,
$\mathcal{T}_i$ denotes the number of timepoints in the $i$th dataset, $t(j)$ denotes the $j$th timepoint, $x_{ij}^{data}$ denotes the value of the $i$th dataset at the $j$th timepoint, and $x_{i}^{sim}|_{t(j)}$ dneotes the simulated value of the metabolite corresponding to the $i$th dataset, interpolated to the $j$th timepoint.
...
\end{equation}
where $\mathcal{P}$ denotes the number of parameters, equal to 815, which includes 163 rate constants, 163 enzyme degradation rate constants, 455 saturation constants, and 34 control parameters, $k_i^{new}$ denotes the new value of the $i$th parameter, $k_i$ denotes the current value of the $i$th parameter, $a$ denotes a distribution variance, 
%set to 0.03, 
and $r_i$ denotes a random sample from the normal distribution.
...
The array of sensitivity indices was organized into a clustergram (Fig.~\ref{fig:ClusterCAT}) with x-axis corresponding to the 163 rate constants in order, and y-axis corresponding to the 163 rate constants rearranged from least sensitive (top) to most sensitive (bottom).
A dendrogram (left) illustrates a hierarchy of rate constants with similar effect on CAT production, separating them into high responders (dark gray background), medium responders (medium gray background), and low responders (light gray background).
...
This array of sensitivity indices was also organized into a clustergram (Fig.~\ref{fig:ClusterSystem}), with corresponding dendrogram (left).
...
The specific glucose uptake rate was constrained to allow a maximum flux of 40 mM/hr according to literature data; the specific amino acid uptake rates were also bound to allow a maximum flux of 30 mM/hr, but did not reach this maximum flux.
...
An ensemble of flux distributions was calculated for 100 sets by randonmly sampling. 
...
The solution flux vector was used to calculate the theoretical carbon yield of CAT for four different cases.
...
For the unconstrained case, all rates were left unbounded. 
...
For the limited oxidative phosphorylation case, an ensemble of flux distributions was generated by randomly sampling the maximum specific oxygen uptake rate to from 0 to 10 mM/hr.
For the TXTL case (constrained by transcription and translation rates), an ensemble was generated by randomly sampling RNAP polymerase levels, ribosome levels, and elongation rates in a physiological range determined from literature as well as the specific glucose uptake rate from 30 to 40 mM/hr. 
For the case where the flux was constrained by experimental data, the lower and upper bounds where data was avaible were randomly sampled in the physiological range of the data in addition to randomly sampling RNAP polymerase levels, ribosome levels, and elongation rates in a physiological range determined from literature. 
\clearpage
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{./Figures/CAT_prod_yield.pdf}
\caption{Sequence specific flux balance analysis of CAT production and yield. A. 95\% confidence interval of the ensemble (light blue region) for CAT concentration versus time. B. Theoretical maximum carbon yield of CAT calcualted by ssFBA for four different cases: unconstrained (black), limited oxidative phosphorylation activity (gray), constrained by transcription and translation rates (light grey), and constrained by experimental data (white). Error bars represent standard deviation of the ensemble.}
...
%\begin{table}[ht]
%\centering
%\includegraphics[width=0.94\textwidth]{./Figures/AIC.pdf}
%\caption{}
%\label{fig:AIC}
%\end{table}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.94\textwidth]{./Figures/ClusterCAT161.pdf}
\caption{Normalized first-order and pairwise sensitivities of CAT production to rate constants. Dark pixels indicate the most sensitive indices. Dendrogram indicates high responders (dark gray background), medium responders (medium gray background), and low responders (light gray background).}
\label{fig:ClusterCAT}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=1.00\textwidth]{./Figures/ClusterSystem.pdf}
\caption{Normalized first-order and pairwise sensitivities of system state to rate constants. Dark pixels indicate the most sensitive indices. Dendrogram indicates high responders (dark gray background), medium responders (medium gray background), and low responders (light gray background).}
\label{fig:ClusterSystem}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=1.00\textwidth]{./Figures/AA_bounded_bar.pdf}
\includegraphics[width=1.00\textwidth]{./Figures/AA_bounded_scatter.pdf}
\caption{Total-order global sensitivities of protein export flux to amino acid import flux upper bounds. Each upper bound was varied from 0 to 0.5 mM/hr.}
\label{fig:AAs_bounded}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=1.00\textwidth]{./Figures/AA_unbounded_bar.pdf}
\includegraphics[width=1.00\textwidth]{./Figures/AA_unbounded_scatter.pdf}
\caption{Total-order global sensitivities of protein export flux to amino acid import flux upper bounds. Each upper bound was varied from 0 to 0.5 mM/hr, except the upper bound with respect to which sensitivity was calculated; this was varied from 0 to 10 mM/hr.}
\label{fig:AAs_unbounded}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1.00\textwidth]{./Figures/C_Secretion.pdf}
\caption{Total-order global sensitivities of protein export flux to glutamic acid, oxygen and glucose import flux upper bounds, and translation initiation rate upper bound. Lactate, ethanol, acetate, and formate export fluxes were held constant at 0.1 mM/hr in the control case. One of these export fluxes was held constant at 10 mM/hr, while the others were held constant at 0.1 mM/hr, in each of the other four cases.}
\label{fig:C_Secretion.pdf}
\end{figure}
